...
module.exports = {
  name: 'chatgpt',
  description: 'Interact to the latest model ChatGpt.',
  usage: 'chatgpt <query>',
  author: 'Jay Mar',
  async execute(senderId, args, pageAccessToken) {
    if (args.length === 0) {
      await sendMessage(senderId, {
        text: 'Usage: chatgpt [your_question]\nExample: chatgpt what is artificial intelligence?'
      }, pageAccessToken);
      return;
    }

    const query = args.join(' ');
    const apiUrl = `${api.jaymar}/api/chatgpt?input=${encodeURIComponent(query)}`;
